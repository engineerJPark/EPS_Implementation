...
def get_eps_loss(cam, saliency, num_classes, label, tau, lam, intermediate=True):
    ...
    sal_pred = F.softmax(cam, dim=1) # channel dimension으로 softmax

    iou_saliency = (torch.round(sal_pred[:, :-1].detach()) * torch.round(saliency)).view(b, num_classes, -1).sum(-1) / \
                   (torch.round(sal_pred[:, :-1].detach()) + 1e-04).view(b, num_classes, -1).sum(-1) # 왤까

    valid_channel = (iou_saliency > tau).view(b, num_classes, 1, 1).expand(size=(b, num_classes, h, w)) # 논문 공식 참조

    label_fg_valid = label_map & valid_channel

    label_map_fg[:, :-1] = label_fg_valid
    label_map_bg[:, :-1] = label_map & (~valid_channel) # 논문 공식 참조
    ...
    sal_pred = fg_map * lam + bg_map * (1 - lam) # 논문 공식 참조
