    ...
    for i, image in enumerate(multi_scale_flipped_image_list):
        with torch.no_grad():
            ...
            if network_type == 'cls':
                ...
                if i % 2 == 1:
                    cam = np.flip(cam, axis=-1) # because of flip trick
                cam_list.append(cam)
                ...
                if i % 2 == 1:
                    cam_fg = np.flip(cam_fg, axis=-1) # because of flip trick
...
def infer_cam_mp(process_id, image_ids, label_list, cur_gpu):
    ...
    for i, (img_id, label) in enumerate(zip(image_ids, label_list)):
        ...
        if args.network_type == 'cls':
            sum_cam = np.sum(cam_list, axis=0) # sum up by batch for classification
        elif args.network_type == 'eps':
            cam_np = np.array(cam_list)
            cam_fg = cam_np[:, 0]
            sum_cam = np.sum(cam_fg, axis=0) # CAM for foreground image 
        else:
            raise Exception('No appropriate model type')
        norm_cam = sum_cam / (np.max(sum_cam, (1, 2), keepdims=True) + 1e-5) # Normalize CAM iamge
